...
export default function VelzonThemeFormModal({
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => {
      const modalRoot = document.getElementById("velzon-theme-form-modal");
      const scrollParent =
      ...
      const first =
        ...
        scrollParent.querySelector<HTMLElement>('[aria-invalid="true"]');
    ...
    >
      ...
      {banner !== null && (
        <Alert variant="success" className="rounded-0 mb-0 border-0">
          {banner ?? (
            <p className="mb-0">
              Please complete all fields below. Required items are validated on submit.
            </p>
          )}
        </Alert>
      )}
