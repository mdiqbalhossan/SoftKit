...
const SearchOption = () => {
    ...
    useEffect(() => {
        const filteredMenuItems = navData.reduce(
            (result: any, menuItem: any) => {
                if (menuItem.isHeader) {
                    return result;
                }
                const lowercaseLabel = menuItem.label
                    ? menuItem.label.toLowerCase()
                    : "";
                const lowercaseLink = menuItem.link
                ...
                const filteredSubItems = (menuItem.subItems || []).filter(
                    (subItem: any) => {
                        const lowercaseSubItemLabel = subItem.label
                            ? subItem.label.toLowerCase()
                            : "";
                        const lowercaseSubItemLink = subItem.link
                            ? subItem.link.toLowerCase()
                            : "";

                        return (
                            lowercaseSubItemLabel.includes(
                                searchTerm.toLowerCase(),
                            ) ||
                            lowercaseSubItemLink.includes(
                                searchTerm.toLowerCase(),
                            )
                        );
                    },
